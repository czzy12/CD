...
系统复制文本在“本人分析/个人分析/公司意见/意见/结论”等人工判断边界停止，包括标题同行已有分析或意见正文的情况；这些内容不得进入客户字段、客户申报或经营上下文。未纳入白名单的“生活轨迹、流水问题”等标签及其内容同样不读取；空白客户字段不得把紧随其后的未知标签误吞为字段值。客户经理描述字段即使位于系统页面中，也按 `customer_manager_description` 单独标记为未核实描述，不得标成客户自述。输出只包含流水搜索所需的姓名/主体、单位、明确行业、工作/居住/上牌地点、车型和经销商等最小上下文及来源状态，不写入 `Transaction`，也不自动进入当前标准结果 JSON。后续关键词、AI 经营关联和申报对照只能读取该上下文与 `original_transactions`，不得直接从 TXT 或 PDF 自由推断。
...
案件经营上下文优先读取标准“行业/经营行业/主营业务”字段，以及“工作介绍及收入情况（是否和流水匹配）”、业务人员备注或客户经理备注中明确、具体的工作和经营内容。后两类属于客户经理未核实描述，不等同于客户本人申报；提取结果须保存原始文字、来源字段、来源文件、来源角色和 `declared_unverified` 状态。“本人分析/个人分析”、公司意见、审批或调查判断不得进入客户信息或经营上下文；“做生意、自己经营、其他生意”等宽泛描述，以及信用卡用途、日常消费、家庭情况、位置轨迹和其他无关备注同样不得进入经营上下文。
...
- `large_inflow_balance_paths`：收入 `>=30000` 元时，只在可靠且相同的 `source_file_id` 内展示入账前余额及其后1/3/7日路径；缺少来源文件ID时输出 `source_file_id_unavailable`，不得把未知来源合并。混合存在可用与缺来源入账时，必须同时输出达到阈值数、成功构造数、缺来源跳过数和部分可用状态，不得静默忽略。参与累计的支出单笔须达到 `max(1000元, 入账额5%)`。累计比例100%为精确同额、90%-110%为近似总额、80%以上为短期大部分转出；窗口日末余额取同来源目标日结束前最后一笔可得余额，目标日无交易时沿用此前最近余额快照。留存比例固定为 `max(日末余额-入账前余额, 0) / 入账额`，在短期大部分转出成立且该比例不超过20%时标记低留存候选。所有标签必须同时输出公式输入和比例，且 `fund_source_attribution` 固定为 `false`。
...
- `top_counterparties`：按可靠可识别名称优先、完整可靠账号其次，分别输出收入和支出 Top 5、可识别对手金额覆盖、占可识别金额比例、占全部方向金额比例、月份和证据交易ID；空值、掩码、占位和明显字母数字前缀拼接短姓名不进入排名。结构化结果需要使用完整可靠账号时仍保留精确值，Markdown只显示账号尾号。
...
上述观察全部直接读取传入的原 `Transaction` 列表，证据 ID 与 `original_transactions` 一致；不得把短期金额/余额共现解释为某笔支出的资金来源。第一版只展示1/3/7日窗口路径，不计算或断言某笔资金实际停留时长。Markdown验收视图须逐笔展示大额交易及大额入账路径的金额、比例、余额、规则标签和证据；账号只显示尾号。

余额、结息和月度变化验收视图复用既有 `income_continuity`、`cashflow_scale_and_recent_change` 和 `end_of_day_balance_and_interest`：展示自然月连续性、全期间月均收支、近3个月与此前3个月变化、逐来源日末余额最低/中位/平均/期末值、逐笔结息及季度变化。微信等来源无余额字段时输出 `balance_not_applicable`；无结息命中必须区分可靠文字字段内已搜索无命中与文字字段不可用。不得把这些数值解释为趋势、平均存款本金、资金充足或偿债能力。
...
schema 1.13 新增 `sensitive_transaction_context_candidates`：

- 仅从 `controlled_keyword_candidates` 已完成的可靠字段扫描中提取敏感组，不重新读取PDF或建立第二套交易记录。
- 固定输出受控词组、命中字段、完整可靠文字上下文、方向、金额、余额、来源、当前交易覆盖期、交易ID和页/行证据；当前交易覆盖期由各来源首末交易推得，不能冒充原件声明期间。
- 按来源输出有效交易数、可靠字段可搜索笔数、候选笔数和不可用原因；无命中与可靠文字字段不可用必须分开。
- 不对“抵、押、借、贷、租、融、资、医、法”等单字无条件匹配；命中不表示真实借贷、抵押、诉讼、医疗事实、异常、欺诈或准入结论。

schema 1.14 收口 `declaration_flow_cross_checks`：

- 自动对照范围固定为工作单位、申报行业或经营内容、下定相关流水。可靠字段命中下定、订金、定金、购车款、首付款、补款或问界时，收入和支出均直接列示；问界及上述明确词统一计入下定总数，同时保留订金、定金等子类。
- 工作地点、住家地址、车辆上牌地点、经销商、门店和下定描述进入 `display_only_items`，只展示系统值、来源角色、来源引用和核实状态，不与流水匹配，不生成不一致结论。车型保留在案件上下文中供后续用车轨迹使用，不在当前申报对照中单独展示。
- 工作地点和住家地址保留为未来生活轨迹模块的系统申报锚点；schema 1.14 不启动生活轨迹判断。
- 自动对照输出逐来源可靠文字搜索覆盖和当前交易覆盖期；未发现只表示当前期间和可靠字段内未命中，不证明申报虚假。

schema 1.15 新增 `manual_verification_questions`：

- 只根据既有确定性观察生成待人工核实事项，不重新解析PDF，不调用外部模型。
- 每项包含稳定问题ID、问题类型、问题正文、触发原因、触发观察、核实要点、待核实状态、交易证据和来源文件ID；同类敏感文字和低留存路径合并，不逐笔制造重复问题。
- 工作地点、住家地址、车辆上牌地点、经销商、门店和下定描述等 `display_only_items` 不触发问题；可靠流水字段已直接命中下定相关词时不再生成“是否对应本次购车”的二次确认问题。系统明确尚未下定时，不因未发现相关流水生成问题。
