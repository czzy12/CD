...
2026-07-30 在接入经营关联、下定购车、交易对手和资金余额前完成小范围整理：

- `CaseDashboardPage`继续作为唯一案件总览页；左侧删除可见的重复“分析结果”入口，只保留“当前案件”分组下的案件概览、人工核实和证据中心。历史 `analysis` 路由只作兼容重定向，不创建第二套 Dashboard。
- 六项关键数据合并为单一 `KeyMetricsPanel`。面板只有一个外边框和硬阴影，内部六项按宽度在6列、3列或2列间自适应；金额继续使用万元缩写和完整金额悬停提示，统计项不提供跳转。
- 分析摘要卡删除视觉编号和 `ANALYSIS` 标识，统一为标题、主要值、摘要、状态和详情按钮，并按“优先处理 / 业务与购车 / 资金情况”分组。资金观察和余额月度继续共用同一模块详情框架，为后续合并成“资金与余额概览”保留条件。
- 证据总表移出普通业务卡，改为 Dashboard 底部证据完整性摘要，分别说明唯一交易索引、有效证据引用、悬空引用和歧义引用数量；入口仍进入统一证据中心。
- 案件概览及没有交易选择的页面继续关闭 `EvidencePanel`；人工核实、敏感候选或证据中心选中具体交易时才展开。

韩鹏飞真实案例沿现有 Worker 链路复核：3个来源、3135笔交易、4项人工核实、21项敏感候选；证据摘要为3135笔交易已建立唯一索引、39431条有效证据引用、0条悬空引用、0条歧义引用。全量单元测试248项、语法检查和离屏GUI启动通过。schema 1.16、Worker、解析后端和业务规则均未修改。

### 1.4 经营关联详情接入状态

2026-07-30 完成经营关联的只读 GUI 接入：

- 经营关联摘要卡不再把“AI 不可用”等同于“全部经营观察不可用”。卡片主要数字优先展示 schema 1.16 中已有的确定性命中与已接受 AI 观察数量，状态条另行说明 AI 可用状态及原因。
- 统一 `ModuleDetailPage` 新增经营关联分页表，列示判断来源、关联分类、证据强度、交易日期、方向、金额、交易对手和判断依据；列表仍只保存标准结果引用和分页行序号，不创建平行交易模型。
- 主候选表只展示 `deterministic_candidates` 与已通过后端校验的 `ai_candidates`。`deterministic_non_business_candidates` 只在状态摘要中显示数量，避免排除项淹没需复核的关联线索。
- 点击任一带 transaction_id 的经营关联行，继续经 schema 1.16 的唯一证据索引打开共享 `EvidencePanel`；GUI 不重算关联规则。
- AI 未授权、留存策略未确认、经营上下文待确认、提供方不可用或返回校验失败等原因均使用中文说明。即使 AI 不可用，已有确定性命中仍单独展示；基础 Worker 继续固定 `ai_config={}` 和 `ai_evaluator=None`。

韩鹏飞真实案例沿 Worker 链路复核：3个来源、3135笔交易；经营关联为47笔确定性命中、0笔AI观察、251笔确定性排除，AI状态保持`business_context_confirmation_required`；经营关联主表47行，首行可展开交易证据。证据完整性仍为3135笔唯一交易索引、39431条有效引用、0条悬空引用、0条歧义引用。全量单元测试252项、语法检查和离屏GUI启动通过。

### 1.5 经营关联分析前确认与人工上下文闭环

2026-07-30 暂停下定购车接入，优先补齐经营上下文：

- 新建案件流程改为“选择目录 → 快速扫描TXT及文件范围 → `CasePreparationPage` → 确认后完整解析 → 案件概览”。确认页展示工作单位、明确工作内容、原始文字/来源和当前上下文状态。
- 人工可填写实际主要经营内容、主要产品或服务、补充说明和确认人；资料不足时主要经营内容必填。可确认开始、暂不补充继续，或返回目录选择。
- 已提取信息与人工确认表单使用左右两栏；AI拟提交字段说明保留完整纵向空间。确认人明确标为必填，缺失时输入框红框聚焦并显示“无法开始分析”原因，避免误解为按钮无响应。
- 人工输入不写回客户TXT、PDF或流水，独立保存为案件级`manual_case_context.json`，包含原始提取信息、人工确认、来源、状态、确认人和UTC确认时间；打开已有案件或重新进入确认页时自动恢复。
- 只有单位名称时，经营卡显示“需补充经营内容 / 当前只有工作单位名称，无法确认实际主营 / 补充经营信息”。暂不补充时，其他模块继续运行；经营模块只列“确定性文字/名称候选”，并说明“经营上下文待确认，未执行行业语义判断”。
- 人工修改后，通过“应用并重新分析经营关联”基于内存交易或schema 1.16既有`original_transactions`恢复值，只重建经营关联、申报对照、相关人工核实和证据审计；不重新解析PDF，也不改写或重新生成`original_transactions`。
- AI复选框默认关闭，并展示拟提交字段范围。未勾选时GUI固定传入禁用配置且不装载AI环境配置；只有用户勾选并点击确认后，才允许装载显式提供方配置。密钥不写入人工上下文文件或标准结果。
- 勾选AI后显示“DeepSeek API Key（仅本次运行）”密码输入框。该值优先于当前进程环境，仅在内存中交给适配器，不持久化；解决从PowerShell启动时环境变量未进入GUI进程导致持续`ai_api_key_missing`的问题。

韩鹏飞真实案例在临时案件工作区完成前后验收：快速扫描识别工作单位“新疆国物能源产业发展有限公司”，没有提取到明确工作内容，状态为“只有工作单位名称，需补充实际经营内容”；暂不补充结果保持47笔确定性文字/名称候选、0笔AI观察。使用明确标注为非案件事实的验收值测试持久化和局部重建后，`original_transactions`仍为原3135笔且逐项不变；证据仍为3135笔唯一索引、39431条有效引用、0悬空、0歧义。全量单元测试260项、语法检查和离屏GUI启动通过，全程未调用外部模型。
