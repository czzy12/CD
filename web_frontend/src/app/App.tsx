  ...
  useEffect(() => {
    ...
    const poll = async () => {
      ...
      if (response.data.state === "completed" && response.data.result_ready) {
        ...
        if (stopped || !headerResponse.ok || !headerResponse.data || headerResponse.data.case_session_id !== response.data.case_session_id) {
          if (!stopped) setError(headerResponse.error?.message || "分析结果已生成，但工作台加载失败");
          return;
        }
