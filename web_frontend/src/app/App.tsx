...
import type {
  AiRuntimeStatusDTO, AnalysisStatusDTO, AppStateDTO, CaseHeaderDTO, CasePreflightDTO, CaseSelectionDTO, DesktopBridge,
...
export function App() {
  ...
  const [settingsContext, setSettingsContext] = useState<ManualContextDTO | null>(null);
  const [settingsDraft, setSettingsDraft] = useState<ManualContextInput>({
    company_name: "", confirmed_primary_business: "", confirmed_products_or_services: "",
    confirmation_note: "",
  });
  ...
  const openResult = async () => {
    ...
    setPreflight(null); setAnalysis(null); setHistoryOpen(false); setSettingsOpen(false); analysisTaskRef.current = null;
    setContextDraft({ company_name: "", confirmed_primary_business: "", confirmed_products_or_services: "", confirmation_note: "" });
    setContextExtract(null);
    setContextNotice("");
    await activateCase(bridge, response.data);
  };

  const prepareCase = async (activeBridge: DesktopBridge, selected: CaseSelectionDTO) => {
    const inspected = await activeBridge.inspectCaseDirectory(selected.case_handle);
    if (!inspected.ok || !inspected.data) {
      setError(inspected.error?.message || "来源预检失败"); setLoading(false); return;
    }
    setPreflight(inspected.data); setAnalysis(null); setHistoryOpen(false); setSettingsOpen(false); analysisTaskRef.current = null; setLoading(false);
    setContextDraft({ company_name: "", confirmed_primary_business: "", confirmed_products_or_services: "", confirmation_note: "" });
    setContextExtract(null);
    setContextNotice("");
    const contextResponse = await activeBridge.getManualCaseContext(selected.case_handle);
  ...
  const chooseNewCase = async () => {
    ...
    await prepareCase(bridge, selected.data);
  };
  ...
  const openHistory = async () => {
    if (!bridge) return;
    setError(""); setHistoryLoading(true); setHistoryOpen(true); setSettingsOpen(false);
  ...
  const openRecentCase = async (recordId: string) => {
    ...
    setHistoryOpen(false); setSettingsOpen(false); setPreflight(null); setAnalysis(null); analysisTaskRef.current = null;
    setContextDraft({ company_name: "", confirmed_primary_business: "", confirmed_products_or_services: "", confirmation_note: "" });
    setContextExtract(null);
    setContextNotice("");
    setAppState((current) => current ? { ...current, case_loaded: true, case_session_id: response.data!.case_session_id, case_revision: response.data!.case_revision } : current);
  ...
  const openSettings = async () => {
    ...
    setSettingsDraft({
      ...
      confirmed_primary_business: response.data.confirmed_primary_business,
      confirmed_products_or_services: response.data.confirmed_products_or_services,
      confirmation_note: response.data.confirmation_note,
    });
    const aiResponse = await bridge.getAiRuntimeStatus();
    if (aiResponse.ok && aiResponse.data) setSettingsAiStatus(aiResponse.data);
  };
  ...
  const saveSettingsContext = async () => {
    ...
    const refreshed = await bridge.getCurrentManualCaseContext();
    if (refreshed.ok && refreshed.data) setSettingsContext(refreshed.data);
  };
  ...
  const clearSettingsContext = async () => {
    ...
    if (refreshed.ok && refreshed.data) {
      setSettingsContext(refreshed.data);
      setSettingsDraft({ company_name: "", confirmed_primary_business: "", confirmed_products_or_services: "", confirmation_note: "" });
    }
    setSettingsNotice("经营上下文已清空");
  };

  const reanalyzeRecentCase = async (recordId: string) => {
    if (!bridge || historyLoading) return;
    setHistoryLoading(true); setError("");
    const response = await bridge.reanalyzeRecentCase(recordId);
    setHistoryLoading(false);
    if (!response.ok || !response.data) {
      setError(response.error?.message || "重新分析失败");
      return;
    }
    setLoading(true);
    await prepareCase(bridge, response.data);
  };
  ...
  const closeCase = async () => {
    ...
    setHistoryOpen(false); setSettingsOpen(false); setSettingsContext(null); setSettingsNotice(""); setRecentCases([]); setRecentCorrupt(false);
    setContextDraft({ company_name: "", confirmed_primary_business: "", confirmed_products_or_services: "", confirmation_note: "" });
    setContextNotice("");
  ...
  const selectModule = (descriptor: ModuleDescriptorDTO) => {
    if (bridge && sessionRef.current) { setSettingsOpen(false); void loadModule(bridge, descriptor, sessionRef.current, 1, {}, pageSize); }
  };

  const selectItem = useCallback(async (item: ReviewItemDTO) => {
    if (!bridge || !sessionRef.current) return;
    setSelectedId(item.item_id); setInspectorOpen(true); setEvidence(null); setEvidenceError("");
  ...
  return <main className="app-shell" data-theme={theme} data-inspector={inspectorOpen || sourceReviewOpen ? "open" : "closed"}>
    <aside className="sidebar">
      ...
      <section className="sidebar-section module-section"><p>流水核查</p>{modules.map((item) => { const Icon = ICONS[item.icon] || CircleHelp; const disabled = !isModuleEnabled(item.availability); return <button key={item.module_id} className={`sidebar-row ${activeModule?.module_id === item.module_id ? "active" : ""} ${disabled ? "disabled" : ""}`} onClick={() => selectModule(item)} disabled={disabled}><Icon /><span>{item.title}</span>{item.review_count > 0 && <i className="review-dot" />}{item.availability === "not_implemented" ? <em>未实施</em> : item.availability === "unavailable" ? <em>不可用</em> : <b>{header ? item.total_count : ""}</b>}</button>; })}</section>
      <div className="sidebar-bottom"><button className={`sidebar-row ${settingsOpen ? "active" : ""}`} onClick={() => { if (settingsOpen) { setSettingsOpen(false); setError(""); } else void openSettings(); }}><SlidersHorizontal /><span>设置</span></button><button className="sidebar-row" onClick={() => setTheme(nextTheme(theme))}>{theme === "dark" ? <Moon /> : <Sun />}<span>主题：{theme === "dark" ? "深色" : "浅色"}</span></button></div>
    </aside>
    <section className="work-area">
      ...
      {settingsOpen ? <SettingsPage caseName={header?.case_name || preflight?.case_display_name || analysis?.case_display_name || ""} loading={settingsLoading} context={settingsContext} draft={settingsDraft} notice={settingsNotice} error={error} aiStatus={settingsAiStatus} onDraftChange={setSettingsDraft} onSave={saveSettingsContext} onClear={clearSettingsContext} onClose={() => { setSettingsOpen(false); setError(""); }} /> : analysis ? <AnalysisProgress value={analysis} error={error} onCancel={cancelAnalysis} onBack={leaveAnalysis} /> : preflight ? <CasePreflight value={preflight} loading={loading} error={error} onStart={startAnalysis} onReselect={chooseNewCase} onCancel={() => { setPreflight(null); setError(""); }} context={contextDraft} extract={contextExtract} contextNotice={contextNotice} onContextChange={setContextDraft} onSaveContext={saveContext} /> : historyOpen ? <HistoryPage cases={recentCases} corrupt={recentCorrupt} loading={historyLoading} error={error} onOpen={openRecentCase} onRemove={removeRecentCase} onReanalyze={reanalyzeRecentCase} onClose={() => { setHistoryOpen(false); setError(""); }} /> : !header ? <EmptyWorkspace connection={connection} loading={loading} error={error} onNew={chooseNewCase} onOpen={openResult} /> : <>
...
function CasePreflight({ value, loading, error, onStart, onReselect, onCancel, context, extract, contextNotice, onContextChange, onSaveContext }: { value: CasePreflightDTO; loading: boolean; error: string; onStart: () => void; onReselect: () => void; onCancel: () => void; context: ManualContextInput; extract: ManualContextDTO | null; contextNotice: string; onContextChange: (next: ManualContextInput) => void; onSaveContext: () => void }) {
  ...
  return <section className="workflow-page"><header className="workflow-header"><div><span>来源预检</span><h1>{value.case_display_name}</h1><p>只核对正式流程支持的来源，不运行完整流水解析。</p></div><dl><div><dt>支持</dt><dd>{value.supported_source_count}</dd></div><div><dt>不支持</dt><dd>{value.unsupported_source_count}</dd></div><div><dt>预检耗时</dt><dd>{value.elapsed_ms}ms</dd></div></dl></header>{value.warnings.length > 0 && <div className="workflow-warnings">{value.warnings.map((warning) => <p key={warning}>{warning}</p>)}</div>}{error && <div className="inline-error">{error}</div>}<div className="source-table"><div className="source-table-head"><span>来源</span><span>类型</span><span>识别</span><span>大小</span><span>状态</span></div>{value.sources.map((source) => <div className="source-table-row" key={source.source_ref}><span><strong>{source.display_name}</strong>{source.warning && <em>{source.warning}</em>}</span><span>{source.detected_source_type === "pdf" ? "PDF" : source.detected_source_type === "excel" ? "Excel" : source.extension || "其他"}</span><span>{source.detected_bank_type || "未识别"}{source.may_use_generic_fallback ? " · 可能通用回退" : ""}</span><span>{formatBytes(source.size)}</span><span className={source.supported ? "source-ready" : source.initial_status === "context" ? "source-context" : "source-unsupported"}>{source.supported ? "可处理" : source.initial_status === "context" ? "上下文资料" : "将跳过"}</span></div>)}</div>{hasExtract && extract && <section className="context-section extracted-context"><header><span>案件上下文（来自 TXT 自动提取）</span></header>{extract.source_names.length > 0 && <p className="context-note">来源：{extract.source_names.join("、")}</p>}<div className="context-grid"><label>工作单位<strong className="extract-value">{extract.company_name || "—"}</strong></label><label>申报工作描述<strong className="extract-value">{extract.declared_work_description || "—"}</strong></label><label>工作单位候选<strong className="extract-value">{extract.work_units.length ? extract.work_units.join("、") : "—"}</strong></label><label>工作地点<strong className="extract-value">{extract.work_locations.length ? extract.work_locations.join("、") : "—"}</strong></label><label>居住地点<strong className="extract-value">{extract.residence_locations.length ? extract.residence_locations.join("、") : "—"}</strong></label><label>提取状态<strong className="extract-value">{contextStatusLabel(extract.declared_work_status)}</strong></label></div></section>}<section className="context-section"><header><span>经营上下文（可选）</span><button className="property-button" onClick={onSaveContext} disabled={loading}>{loading ? <LoaderCircle className="spin" /> : <Save />}保存经营上下文</button></header>{contextNotice && <p className="context-notice">{contextNotice}</p>}<p className="context-note">当 TXT 未提取到明确工作内容时，可在此人工补充；填写并保存即视为已确认，作为辅助。AI 经营判断仅使用已验收缓存，分析不联网。</p><div className="context-grid"><label>工作单位<input value={context.company_name} onChange={(event) => onContextChange({ ...context, company_name: event.target.value })} /></label><label>主要经营内容<input value={context.confirmed_primary_business} onChange={(event) => onContextChange({ ...context, confirmed_primary_business: event.target.value })} /></label><label>产品 / 服务<input value={context.confirmed_products_or_services} onChange={(event) => onContextChange({ ...context, confirmed_products_or_services: event.target.value })} /></label><label>补充说明<textarea value={context.confirmation_note} onChange={(event) => onContextChange({ ...context, confirmation_note: event.target.value })} /></label></div></section><footer className="workflow-actions"><button className="secondary-button" onClick={onCancel}>取消</button><button className="secondary-button" onClick={onReselect} disabled={loading}>重新选择</button><button className="primary-button" onClick={onStart} disabled={loading || !value.can_start}>{loading ? <LoaderCircle className="spin" /> : <ClipboardCheck />}开始分析</button></footer></section>;
}

function HistoryPage({ cases, corrupt, loading, error, onOpen, onRemove, onReanalyze, onClose }: { cases: RecentCaseDTO[]; corrupt: boolean; loading: boolean; error: string; onOpen: (recordId: string) => void; onRemove: (recordId: string) => void; onReanalyze: (recordId: string) => void; onClose: () => void }) {
  return <section className="workflow-page history-page"><header className="workflow-header"><div><span>历史案件</span><h1>最近打开的案件</h1><p>只记录摘要，不包含客户原始资料与完整结果。</p></div></header>{error && <div className="inline-error">{error}</div>}{corrupt && <div className="workflow-warnings"><p>最近案件索引损坏，已按空列表处理；打开新案件后会重建。</p></div>}{loading ? <div className="loading-line"><LoaderCircle className="spin" />正在读取历史案件…</div> : cases.length === 0 ? <ListState text="暂无历史案件" /> : <div className="history-list">{cases.map((record) => <div className="history-row" key={record.record_id}><div><strong>{record.case_name}</strong><em>{record.period_start || "—"} 至 {record.period_end || "—"} · {record.source_count} 来源 · {record.transaction_count} 交易 · schema {record.schema_version || "—"}</em></div><span>{record.updated_at}</span><div className="history-actions">{record.available ? <button className="property-button" disabled={loading} onClick={() => onOpen(record.record_id)}>打开</button> : <button className="property-button" disabled={loading} onClick={() => onReanalyze(record.record_id)}>重新分析</button>}<button className="property-button danger-text" onClick={() => onRemove(record.record_id)}>删除</button></div></div>)}</div>}<footer className="workflow-actions"><button className="secondary-button" onClick={onClose}>返回</button></footer></section>;
}

function SettingsPage({ caseName, loading, context, draft, notice, error, aiStatus, onDraftChange, onSave, onClear, onClose }: { caseName: string; loading: boolean; context: ManualContextDTO | null; draft: ManualContextInput; notice: string; error: string; aiStatus: AiRuntimeStatusDTO | null; onDraftChange: (next: ManualContextInput) => void; onSave: () => void; onClear: () => void; onClose: () => void }) {
  const hasExtract = Boolean(context && (context.source_names.length > 0 || context.company_name || context.declared_work_description || context.work_units.length > 0 || context.work_locations.length > 0 || context.residence_locations.length > 0));
  return <section className="workflow-page settings-page"><header className="workflow-header"><div><span>设置</span><h1>经营上下文</h1><p>查看当前案件已保存的经营上下文，并可直接补充、修正或清空。</p></div></header>{error && <div className="inline-error">{error}</div>}{notice && <p className="context-notice">{notice}</p>}{loading ? <div className="loading-line"><LoaderCircle className="spin" />正在读取经营上下文…</div> : !context ? <ListState text={caseName ? "当前案件没有关联目录，无法读取经营上下文" : "未加载案件"} /> : <><section className="context-section extracted-context"><header><span>案件上下文（来自 TXT 自动提取）</span></header>{hasExtract ? <><div className="context-grid"><label>工作单位<strong className="extract-value">{context.company_name || "—"}</strong></label><label>申报工作描述<strong className="extract-value">{context.declared_work_description || "—"}</strong></label><label>工作单位候选<strong className="extract-value">{context.work_units.length ? context.work_units.join("、") : "—"}</strong></label><label>工作地点<strong className="extract-value">{context.work_locations.length ? context.work_locations.join("、") : "—"}</strong></label><label>居住地点<strong className="extract-value">{context.residence_locations.length ? context.residence_locations.join("、") : "—"}</strong></label><label>提取状态<strong className="extract-value">{contextStatusLabel(context.declared_work_status)}</strong></label></div>{context.source_names.length > 0 && <p className="context-note">来源：{context.source_names.join("、")}</p>}</> : <p className="context-note">当前案件没有已保存的上下文文件，也未提取到明确内容。</p>}</section><section className="context-section"><header><span>AI 经营判断状态</span></header><div className="context-grid"><label>运行时<strong className="extract-value">{aiStatus?.runtime_loaded ? "已加载" : "未加载"}</strong></label><label>模式<strong className="extract-value">验收缓存离线重放（零网络）</strong></label><label>缓存条目<strong className="extract-value">{aiStatus?.cache_file_count ?? 0}</strong></label><label>模型<strong className="extract-value">{aiStatus?.model || "—"}</strong></label></div><p className="context-note">工作台分析不会联网调用模型；AI 候选只来自已验收缓存覆盖范围，未覆盖时以确定性结果为主。</p></section><section className="context-section"><header><span>人工经营上下文（可编辑）</span><div className="module-actions"><button className="property-button danger-text" onClick={onClear} disabled={loading}>清空经营上下文</button><button className="property-button" onClick={onSave} disabled={loading}>{loading ? <LoaderCircle className="spin" /> : <Save />}保存经营上下文</button></div></header><p className="context-note">填写并保存即视为已确认；保存到案件工作区，不写入客户原始资料；重新构建上下文观察后应用到当前案件。</p><div className="context-grid"><label>工作单位<input value={draft.company_name} onChange={(event) => onDraftChange({ ...draft, company_name: event.target.value })} /></label><label>主要经营内容<input value={draft.confirmed_primary_business} onChange={(event) => onDraftChange({ ...draft, confirmed_primary_business: event.target.value })} /></label><label>产品 / 服务<input value={draft.confirmed_products_or_services} onChange={(event) => onDraftChange({ ...draft, confirmed_products_or_services: event.target.value })} /></label><label>补充说明<textarea value={draft.confirmation_note} onChange={(event) => onDraftChange({ ...draft, confirmation_note: event.target.value })} /></label></div></section></>}<footer className="workflow-actions"><button className="secondary-button" onClick={onClose}>返回</button></footer></section>;
}

const STAGE_LABELS: Record<string, string> = {
  discovering_sources: "正在识别来源", detecting_source_type: "正在识别来源类型",
  parsing_source: "正在解析来源", normalizing_transactions: "正在规范化交易",
  reading_source: "正在处理来源", building_result: "正在构建标准结果（AI 仅离线重放，不联网）",
...
function contextStatusLabel(status: string): string {
  if (status === "declared_unverified") return "已提取（未核实）";
  if (status === "unavailable") return "未提取到明确工作内容";
  return status || "—";
}
function renderEvidenceLines(lines: string[]): string[] {
  const result: string[] = [];
  for (const line of lines) {
    if (line.startsWith("raw_text：")) {
      for (const part of line.slice("raw_text：".length).split("|")) {
