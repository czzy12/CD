...
import type {
  AnalysisStatusDTO, AppStateDTO, CaseHeaderDTO, CasePreflightDTO, CaseSelectionDTO, DesktopBridge,
...
export function App() {
  ...
  return <main className="app-shell" data-theme={theme} data-inspector={inspectorOpen || sourceReviewOpen ? "open" : "closed"}>
    ...
    <section className="work-area">
      ...
      {analysis ? <AnalysisProgress value={analysis} error={error} onCancel={cancelAnalysis} onBack={leaveAnalysis} /> : preflight ? <CasePreflight value={preflight} loading={loading} error={error} onStart={startAnalysis} onReselect={chooseNewCase} onCancel={() => { setPreflight(null); setError(""); }} context={contextDraft} extract={contextExtract} contextNotice={contextNotice} onContextChange={setContextDraft} onSaveContext={saveContext} /> : historyOpen ? <HistoryPage cases={recentCases} corrupt={recentCorrupt} loading={historyLoading} error={error} onOpen={openRecentCase} onRemove={removeRecentCase} onReanalyze={reanalyzeRecentCase} onClose={() => { setHistoryOpen(false); setError(""); }} /> : settingsOpen ? <SettingsPage caseName={header?.case_name || ""} loading={settingsLoading} context={settingsContext} draft={settingsDraft} notice={settingsNotice} error={error} onDraftChange={setSettingsDraft} onSave={saveSettingsContext} onClear={clearSettingsContext} onClose={() => { setSettingsOpen(false); setError(""); }} /> : !header ? <EmptyWorkspace connection={connection} loading={loading} error={error} onNew={chooseNewCase} onOpen={openResult} /> : <>
...
function SettingsPage({ caseName, loading, context, draft, notice, error, onDraftChange, onSave, onClear, onClose }: { caseName: string; loading: boolean; context: ManualContextDTO | null; draft: ManualContextInput; notice: string; error: string; onDraftChange: (next: ManualContextInput) => void; onSave: () => void; onClear: () => void; onClose: () => void }) {
